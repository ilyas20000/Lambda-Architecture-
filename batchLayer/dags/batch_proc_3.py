...
def batch_proc_3():
    ...
    for row in cursor.fetchall():
        result_dict = {'p': row[0], 's': row[2], 't': str(datetime.fromtimestamp(round(row[3]/1000.0))), 'v': row[4]}
    ...
    try:
        ...
        with open('/tmp/batchViews/parquet_output_3.parquet', 'x'):
            pass  # This block is empty, creating an empty file

        print(f"File '/tmp/batchViews/parquet_output_3.parquet' created successfully.")
    except FileExistsError:
        print(f"File '/tmp/batchViews/parquet_output_3.parquet' already exists.")

    # Write the result to a Parquet file
    parquet_file_path = '/tmp/batchViews/parquet_output_3.parquet'
    table = pa.Table.from_pandas(result)
    pq.write_table(table, parquet_file_path)

    #read data
    df = pq.read_pandas('/tmp/batchViews/parquet_output_3.parquet').to_pandas()
