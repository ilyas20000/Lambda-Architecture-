...
def batch_proc_2():
    ...
    try:
        ...
        with open('/tmp/batchViews/parquet_output_2.parquet', 'x'):
            pass  # This block is empty, creating an empty file

        print(f"File '/tmp/batchViews/parquet_output_2.parquet' created successfully.")
    except FileExistsError:
        print(f"File '/tmp/batchViews/parquet_output_2.parquet' already exists.")

    # Write the result to a Parquet file
    parquet_file_path = '/tmp/batchViews/parquet_output_2.parquet'
    table = pa.Table.from_pandas(result)
    pq.write_table(table, parquet_file_path)

    #read data
    df = pq.read_pandas('/tmp/batchViews/parquet_output_2.parquet').to_pandas()
